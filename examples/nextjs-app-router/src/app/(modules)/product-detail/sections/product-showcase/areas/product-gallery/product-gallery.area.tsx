...
import styles from './product-gallery.area.module.scss';
...
export function ProductGalleryArea() {
  ...
  return (
    <BaseBox>
      <BaseBox className={styles['product-gallery-main-image']}>
        <img
          src={activeImage.url}
          alt={activeImage.alt}
        />
      </BaseBox>
      <BaseBox className={styles['product-gallery-thumbnails']}>{product.images.map(renderThumbnail)}</BaseBox>
    </BaseBox>
  );
}
