...
import styles from './delivery-info.segment.module.scss';
...
export function DeliveryInfoSegment() {
  ...
  const stockClass =
    deliveryInfo.stockStatus === 'in_stock'
      ? styles['delivery-info-stock-in-stock']
      : deliveryInfo.stockStatus === 'low_stock'
        ? styles['delivery-info-stock-low-stock']
        : styles['delivery-info-stock-out-of-stock'];
  ...
  return (
    <BaseBox>
      {deliveryInfo.isFree && <BaseBox className={styles['delivery-info-free']}>FREE delivery {deliveryInfo.estimatedDate}</BaseBox>}
      <BaseBox className={styles['delivery-info-details']}>
        Or fastest One-Day delivery {deliveryInfo.fastestOption}. Order within {deliveryInfo.orderWithin}
      </BaseBox>
      <BaseBox className={styles['delivery-info-location']}>Delivering to {deliveryInfo.location}</BaseBox>
      <BaseBox className={`${styles['delivery-info-stock']} ${stockClass}`}>{stockLabel}</BaseBox>
    </BaseBox>
  );
}
